...
static int eat_whitespace(FILE *f, int echo_comments) {
  ...
  do {
    ...
    if (c == '#') { /* # begins comments that extend to the newline */
      if (echo_comments) putc(c, stdout);
      do {
        c = getc(f);
        if (echo_comments) {
          if (c != EOF)
...
static int eat_plus(FILE *f) {
  int c = getc(f);
  if (c != EOF && c != '+') ungetc(c, f);
  return (c == '+' || c == '-');
}

static int eat_i(FILE *f) {
  int c = getc(f);
  if (c != EOF && tolower(c) != 'i') ungetc(c, f);
  return (tolower(c) == 'i');
}
...
enum { SORT_FREQUENCY, SORT_DECAY, SORT_ERROR, SORT_AMPLITUDE, SORT_Q } sortby = SORT_FREQUENCY;
...
static int compar(const void *a, const void *b) {
  ...
  switch (sortby) {
    case SORT_FREQUENCY: return cmp(harminv_get_freq(hd, *ia), harminv_get_freq(hd, *ib));
    case SORT_DECAY: return cmp(harminv_get_decay(hd, *ia), harminv_get_decay(hd, *ib));
...
static int mode_ok(harminv_data d, int k, void *ok_d_) {
  ...
  if (k == -1) { /* initialize */
    int i;
    ok_d->num_ok = 0;
    if (!harminv_get_num_freqs(d)) return 0;
    ...
    for (i = 1; i < harminv_get_num_freqs(d); ++i) {
      double err, amp;
      if ((err = harminv_get_freq_error(d, i)) < ok_d->min_err) ok_d->min_err = err;
      harminv_get_amplitude(&aa, d, i);
      if ((amp = cabs(aa)) > ok_d->max_amp) ok_d->max_amp = amp;
    }
    return 0;
  }
  else if (k == -2) { /* finish */
    if (ok_d->verbose && harminv_get_num_freqs(d))
      printf("# harminv: %d/%d modes are ok: "
             ...
             "amps >= %g, %e * %g, "
             "|Q| >= %g\n",
             ok_d->num_ok, harminv_get_num_freqs(d), ok_d->err_thresh, ok_d->rel_err_thresh,
             ok_d->min_err, ok_d->amp_thresh, ok_d->rel_amp_thresh, ok_d->max_amp, ok_d->Q_thresh);
    return 0;
  }
  ...
  ok = ((!ok_d->only_f_inrange || (f >= ok_d->fmin && f <= ok_d->fmax)) &&
        ...
        ampk >= ok_d->amp_thresh && ampk >= ok_d->rel_amp_thresh * ok_d->max_amp &&
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "hvVTFwnt:d:f:s:e:E:a:A:Q:")) != -1)
    switch (c) {
      ...
      case 'T': specify_periods = 1; break;
      case 'w': specify_omega = 1; break;
      case 'n': negate_omega = 1; break;
      case 'F': ok_d.only_f_inrange = 1; break;
      case 'a': ok_d.rel_amp_thresh = atof(optarg); break;
      case 'A': ok_d.amp_thresh = atof(optarg); break;
      case 'E': ok_d.err_thresh = atof(optarg); break;
      case 'e': ok_d.rel_err_thresh = atof(optarg); break;
      case 'Q': ok_d.Q_thresh = atof(optarg); break;
      case 't': dt = atof(optarg); break;
      case 'f': nfmin = atoi(optarg); break;
      case 'd':
        density = atof(optarg);
        if (density < 0) {
      ...
      case 's':
        switch (tolower(optarg[0])) {
          case 'f': sortby = SORT_FREQUENCY; break;
          case 'd': sortby = SORT_DECAY; break;
          case 'e': sortby = SORT_ERROR; break;
          case 'a': sortby = SORT_AMPLITUDE; break;
          case 'q': sortby = SORT_Q; break;
          default:
  ...
  if (verbose) printf("# harminv: %d inputs, dt = %g\n", n, dt);
  ...
  for (iarg = optind; iarg < argc; ++iarg) {
    ...
    if (negate_omega) dt *= -1;
    if ((fmin > fmax && dt > 0) || (fmin < fmax && dt < 0)) {
      double dummy = fmin;
      fmin = fmax;
      fmax = dummy;
    }
    if (verbose) printf("# searching frequency range %g - %g\n", fmin, fmax);
    ...
    for (i = 0; i < harminv_get_num_freqs(hd); ++i) {
      ...
      printf("%g, %e, %g, %g, %g, %e\n", freq * (specify_omega ? TWOPI : 1.0), decay,
             harminv_get_Q(hd, j), cabs(amp), carg(amp) * (dt < 0 ? -1 : 1), err);
    }
